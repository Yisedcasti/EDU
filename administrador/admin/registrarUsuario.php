...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['foto_perfil'])) {
        ...
        if (move_uploaded_file($tmp_name, $directorio . $foto_perfil_new_name)) {
            ...
            try {
                $sentencia = $base_de_datos->prepare("INSERT INTO registro (rol_id_rol, tipo_doc, num_doc, nombres, apellidos, cel, telefono, direccion, correo, foto_perfil, contraseña, jornada_id_jornada) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
                ...
                if ($resultado) {
                    ...
                    echo "Algo salió mal. Por favor verifique los datos y vuelva a intentarlo.";
                }

            } catch (PDOException $e) {
          echo "<script>alert('Algo salió mal. Por favor verifique los datos y vuelva a intentarlo.');</script>". $e->getMessage();
            }
        } else {
